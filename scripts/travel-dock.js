...
const DOCK = {
  ...
  headerFallback: 64,
  // The bottom tray extends this far past the layout-viewport bottom so the iOS
  // Safari toolbar gap (where fixed `bottom: 0` sits above the visible bottom)
  // can never expose page content beneath the clip.
  bottomBuffer: 180,
};
...
export function initTravelDock() {
  ...
  const place = () => {
    ...
    if (dockTop) {
      ...
      tray.style.top = "auto";
      tray.style.bottom = `${-DOCK.bottomBuffer}px`;
